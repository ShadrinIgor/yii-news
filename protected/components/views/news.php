<div class="cItem listItems">
    ...
    <div class="TN_img"><div class="count_shows">Просмотров <?= $values->col ?></div><img alt="<?= $values->name ?> - <?= $values->cid_id->name ?> <?= $values->country->name ?>" title="<?= $values->name ?> - <?= $values->cid_id->name ?> <?= $values->country->name ?>" src="<?= ImageHelper::getImage( $values->image, 2, $values ) ?>">|<?= $values->image ?>|</div>
